...
function InitWebEmailMethodsInsideScopeDEWA(scope) {
    ...
    scope.newIntraction = function (scope, data) {
        ...
        if (scope.getInteraction(scope) == null) {
            scope.WriteLog(INFO, "newIntraction --> New WebEmail Interaction. IntractionID - " + scope.interactionID + ". State - " + data.state);
    ...
    //Load Initilize
    scope.initializeWebEmailLayout = function (scope, data) {

        scope.WriteLog(DEBUG, "initializeWebEmailLayout -->  Start.");

        //Default CRM Maximize or Minimize
        if (scope.defaultMinimizeCRM == true)
            scope.btnWebEmailMinimizeCRM();
        else
            scope.btnWebEmailMaximizeCRM();
        ...
        scope.WriteLog(DEBUG, "initializeWebEmailLayout -->  End.");
    };
    ...
    scope.loadWidgetDataContextStore = function (scope, data) {
        ...
        scope.widgetCallType = '';
        scope.accountNumber = '';

        if (scope.objInteraction.direction == 'INCOMING') {
            scope.widgetCallType = scope.objContextStore.data.CallType;
    ...
    scope.bindRecentCallNotesReasonFilters = function () {
        ...
        scope.WriteLog(DEBUG, 'bindRecentCallNotesReasonFilters --> end.');
    }
    ...
    scope.bindCRMURLWindow = function (scope) {
        ...
        scope.openEmailScreenpopTab(scope, _url);

        scope.WriteLog(DEBUG, 'bindCRMURLWindow --> End. URL :' + _url);

    }

    scope.openEmailScreenpopTab = function (scope, url) {
        ...
        scope.setInteraction(scope);
        scope.WriteLog(DEBUG, "openEmailScreenpopTab - Start for broadcast");
        var json_obj = {
        ...
        scope.broadcast_email_channel.postMessage(json_obj);
        scope.crmURLOpened = true;

        scope.WriteLog(DEBUG, "openEmailScreenpopTab - End");
    };
    ...
    scope.postCSDetailsToIframe = function (scope, iframeName) {
        ...
        scope.WriteLog(DEBUG, "postCSDetailsToIframe --> Jsin data - " + JSON.stringify(reqJson));

        scope.WriteLog(DEBUG, "postCSDetailsToIframe --> message sent!!!. Iframe ID - " + iframeName);
    }
    ...
    scope.bindIframeContactSearch = function (scope) {
        ...
        $(scope.htmlInteractionPrefix + ' #iframeContactSearch').attr('data', _contactURL);
    ...
    scope.onActive = function (scope, api, data) {
        ...
        scope.newIntraction(scope, data);
        scope.initializeWebEmailLayout(scope, data);
    ...
    scope.onACW = function (scope, api, data, $timeout) {
       ...
       // $("#ow_open_Widget_Email").click();
        $timeout(function () {
            angular.element(document.getElementById("ow_open_Widget_Email")).trigger('click');
        })
        scope.WriteLog(DEBUG, 'onAlerting --> End');

    }

    scope.onMediaEvent = function (scope, data) {

        scope.WriteLog(DEBUG, "MediaEvent - Start");

        scope.setEmailContent(scope, data, 'onMediaEvent');

        scope.WriteLog(DEBUG, "MediaEvent - End");
    }

    scope.onMediaMessageEvent = function (scope, data) {

        scope.WriteLog(DEBUG, "onMediaMessageEvent - Start");

        scope.setEmailContent(scope, data, 'onMediaMessageEvent');

        scope.WriteLog(DEBUG, "onMediaMessageEvent - End");
    }

    scope.setEmailContent = function (scope, data, event) {

        scope.WriteLog(DEBUG, 'setEmailContent - Start. Event Type - ' + event + '. Data - ' + JSON.stringify(data));
 
        if (event == 'onMediaMessageEvent') {

            //Displaying the below details on the incoming email screen
            //scope.emailfrom = scope.objCsDetails.meailFrom;
            scope.emailtolist = scope.prepareSemicolonEmailAddress(data.sendTo);

            scope.emailfrom = data.outBoundOriginator;
         
        }
        else if (event == 'onMediaEvent' && data.messages.length > 0) {

            //Send email status reviced 
            var _data = data.messages[0]; 
            scope.emailfrom = _data.outBoundOriginator;
            scope.emailtolist = scope.prepareSemicolonEmailAddress(_data.sendTo); 
        }
          

        scope.WriteLog(DEBUG, 'setEmailContent - End');
        return scope;
    }


    scope.prepareSemicolonEmailAddress = function (addressList) {
        scope.WriteLog(DEBUG, "prepareSemicolonEmailAddress -> Start");
        if (addressList.length == 0) {
            scope.WriteLog(DEBUG, "prepareSemicolonEmailAddress -> End");
            return '';
        }
        ...
        for (i = 0; i < addressList.length; i++) {
            _addressList += addressList[i] + "; ";
        }

        _addressList = _addressList.substring(0, _addressList.length - 2);
        scope.WriteLog(DEBUG, "prepareSemicolonEmailAddress -> End");
        return _addressList;
    }

    //CRM Maximize
    scope.btnWebEmailMaximizeCRM = function () {
    ...
    //CRM Minimize  
    scope.btnWebEmailMinimizeCRM = function () {
    ...
    scope.SearchRecentCallNotes = function (scope) {
        ...
        var _searchValue = $(scope.htmlInteractionPrefix + ' #recentSearchValue');
        _searchValue.removeClass("required-field");

        var _cboRecentSearch = $(scope.htmlInteractionPrefix + ' #cboRecentSearch');
        _cboRecentSearch.removeClass("required-field");

        var _searchKey = scope.cboRecentSearchSelected 
        ...
        if (_searchValue.val().length == 0) {

            _searchValue.addClass("required-field");
        ...
        //New Requirement
        var wsReqJson = JSON.stringify({
            "serviceName": "SearchCallNotes_NonVoice_Email",
            "requestData": reqJson
        });
    ...
    scope.saveRecentCallNotes = function (scope, event) {
        ...
        //New Requirement customWidgetPostRequest
        var wsReqJson = JSON.stringify({
    ...
    scope.getCRMVQueryString = function (scope) {
        ...
        scope.WriteLog(DEBUG, "getCRMWebEmailQueryString - Prepare Query string. Start");

        //https://<<SAP_CRM_URL>>/? sap-agent=1234& sap-interaction_id=xxxxxxxxxxxx&sap-work_req_id =xxxxxxxxxxxxxxx&sap-channel=Email&sap-email=gdevaraju@avaya.com&sap-buag_id=

        var queryString =
            "?sap-agent=" + scope.configuration.handle
            + "&sap-interaction_id=" + scope.objInteraction.id
            + "&sap-work_req_id=" + scope.workRequestId
            + "&sap-channel=" + 'EMAIL'//scope.objInteraction.channel
            + "&sap-email=" + scope.objInteraction.originatingAddress
            + "&saprole=" + scope.QueryStrinSAPRole
            + "&sap-buag_id=";

        scope.WriteLog(DEBUG, "getCRMWebEmailQueryString - End. Query String - " + queryString);
        return queryString;
    ...
    scope.executeWebRequest = function (scope, token, requestUrl, httpMethodType, requestData, serviceType) {
        ...
        scope.WriteLog(DEBUG, 'executeWebRequest -> Start. Service type is  - ' + serviceType + ' HTTP method type - ' + httpMethodType + '. webservice URL - ' + requestUrl + 'Request Data - ' + JSON.stringify(requestData));
    ...
    scope.respSetRecentCallNotes = function (scope, jsonResponse, requestData) {
        ...
        if (jsonResponse == null || jsonResponse == undefined || jsonResponse == '') {
            ...
            scope.focusNativeEmailWidget(scope);
            return;
        }
    ...
    scope.focusNativeEmailWidget =function (scope){

        scope.timeout(function () {
            angular.element(document.getElementById("ow_open_Email")).trigger('click');
        });
    }
    ...
    scope.respSetCallLog = function (scope, jsonResponse, requestData) {
        ...
        scope.WriteLog(DEBUG, "respSetCallLog -> . WebEmail details Updated Successfully " + jsonResponse);
    };
    ...
    scope.respProcessCSNotification = function (scope, jsonResponse, requestData) {
       ...
       if(scope.objContextStore.data != undefined && scope.objContextStore.data !=[]){ 
        ...
        var _lastVerifiedAccount = ((scope.objContextStore.data == null || scope.objContextStore.data.LastVerifiedUsedAcct == undefined) ? '' : scope.objContextStore.data.LastVerifiedUsedAcct);
...
function InitWebEmailLogDEWA(scope) {

    scope.WIDGET_NAME = "WIDGET EMAIL"
